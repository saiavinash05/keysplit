...
def generate_transaction_key() -> str:
    """256‑bit transaction key (hex)."""
    return secrets.token_hex(32)

# Custom Shamir's Secret Sharing implementation
_PRIME = (1 << 521) - 1  # A large prime > 2^256


def split_key(secret_hex: str, num_shares: int = 5, threshold: int = 3) -> List[str]:
    """
    Split the secret_hex into num_shares, with threshold using Shamir's scheme.
    Returns shares as strings "x-y" where y is hex.
    """
    secret_int = int(secret_hex, 16)
    # Generate random coefficients a1..a_{k-1}
...
def reconstruct_key(shares: List[str]) -> str:
    """
    Reconstruct the secret from a list of shares "x-y".
...
def encrypt_share(share: str, pub_key) -> bytes:
    """Encrypt a single share (string) with RSA/OAEP."""
...
# === Existing Signing / Verification Utils ===
...
def verify_response(resp: dict, pub_key) -> bool:
    try:
        print(f"Verifying {resp['entity']} response → {resp['share'].encode()}")
        pub_key.verify(resp['sig'], resp['share'].encode(), padding.PKCS1v15(), hashes.SHA256())
        return True
    except Exception as e:
        print(f"❌ Verify failed for {resp['entity']}: {e}")
        return False
...
# === Phase 2: Request & Collect Shares ===

def request_shares_parallel(user_id: str, user_priv, entities: list, encrypted_shares: dict) -> list:
    """
    Each entity:
      - decrypts its encrypted_shares[id]
      - signs the plaintext share
      - returns {'entity', 'share', 'sig'}
    """
    def proc(ent):
        eid = ent['id']
        # Simulate user request
        _ = sign_request(user_id, user_priv)

        # Entity decrypts its encrypted share
        share_plain = ent['private_key'].decrypt(
            encrypted_shares[eid],
            padding.OAEP(mgf=padding.MGF1(hashes.SHA256()), algorithm=hashes.SHA256(), label=None)
        ).decode()

        # Entity signs back the share
...
if __name__ == '__main__':
    ...
    # Phase 1: generate, split, encrypt
    ...
    # Phase 3: reconstruct with first 3 valid shares
    valid = []
for r in responses:
    if r['entity'] in ['user1', 'user2','user3']:
        if verify_response(r, next(e['public_key'] for e in entities if e['id'] == r['entity'])):
            valid.append(r)
    else:
        print(f"⛔ Ignoring response from {r['entity']} (simulated failure or invalid)")

if len(valid) < 3:
    print('\n🚫 Threshold not met: Only', len(valid), 'valid shares available.')
    print('❌ Cannot reconstruct the transaction key.')
else:
